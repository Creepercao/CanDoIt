...
# CSS properties that are managed by the deck navigation system.
# Stripped from inline styles to prevent LLM-generated slides from
# overriding the deck's show/hide/animation behaviour.
_DECK_MANAGED_PROPS: frozenset[str] = frozenset({
...
_SECTION_STYLE_RE: re.Pattern = re.compile(
    r'(<section\b[^>]*?\bstyle=")([^"]*)(")',
    re.IGNORECASE,
)
...
def _sanitize_slide_html(html: str) -> str:
    """Remove CSS properties that conflict with deck navigation from
    ``<section>`` inline styles.  The deck CSS relies on ``display:none``
    (inactive) / ``display:block`` (active) switching — an inline
    ``display:flex`` (higher specificity) would make the slide permanently
    visible and bleed graphics into neighbouring slides."""
    ...
    def _clean(m: re.Match) -> str:
        ...
        for decl in raw.split(";"):
            ...
            if ":" in decl:
                ...
                if prop not in _DECK_MANAGED_PROPS:
                    kept.append(decl)
            else:
                kept.append(decl)
        if kept:
            return f'{before}{"; ".join(kept)}{after}'
        # Drop the entire style attribute if nothing useful remains
        return before.rstrip(' style="').rstrip('style="') + after.lstrip('"')
...
"""
        ...
        try:
            ...
            # Strip inline styles that would conflict with deck navigation CSS
            html = _sanitize_slide_html(html)
        except Exception as e:
    ...
    results = await asyncio.gather(*[_gen_one(task) for task in my_tasks])
    return {"skill_outputs": {"ppt_slide": list(results)}}
